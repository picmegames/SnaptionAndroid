...
import com.snaptiongame.app.presentation.view.friends.FriendsPresenter;
...
public class FriendsTest {
    ...
    public void filter_isCorrect() throws Exception {
        ...
        assertEquals(FriendsPresenter.filterList(inst, "some filter category"), new
                ArrayList<Friend>());
        ...
        check = FriendsPresenter.filterList(inst, "Steve");
        ...
        check = FriendsPresenter.filterList(inst, "cena");
        ...
        //check not in list
        check.clear();
        assertEquals(FriendsPresenter.filterList(inst, "zzzz"), new ArrayList<Friend>());

        //check both first and last
        check = FriendsPresenter.filterList(inst, "hn ce");
        assertTrue(check.get(0).fullName.equals("John Cena"));
        assertEquals(check.size(), 1);

        //check empty queries
        check = FriendsPresenter.filterList(inst, "");
        assertEquals(check, inst);
        check = FriendsPresenter.filterList(inst, null);
        assertEquals(check, inst);
    }
}
